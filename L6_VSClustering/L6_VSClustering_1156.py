...
def get_web_text(url):
	...
	# Removing scripts
	[tag.decompose() for tag in soup("script")]t
...
class VSCluster():
	...
	def get_min_doc_dist(self, docindex, return_dist = False):
		...
		mind = min(doc_vec)
		mindocs = [sorted(self.clusterlist.keys())[j] for j in range(len(doc_vec)) if doc_vec[j] == mind]
	...
	def cluster(self, return_for_hier = False):
		...
		if self.input_type != "HIER":
			self.clusters = {"Cluster "+str(i):end_clusters[i] for i in range(len(end_clusters))}
		else:
			self.clusters = {"Topic "+str(i):end_clusters[i] for i in range(len(end_clusters))}
	...
	def print_details(self):
		...
		print("The documents are clustered using the Nearest Neighbour method as follows:")
...
if __name__ == "__main__":
	...
	vsobj_hier.fit(wordlist=wordlist, docfile=lvl1_cluslist, input_type="HIER", wordmap=lvl1_clusmap)
	vsobj_hier.cluster()
	...
	vsobj2_hier.fit(wordlist=urlwordlist, docfile=url_cluslist, input_type="HIER", wordmap=url_clusmap)
	vsobj2_hier.cluster()
