...
class CosineSimilarityClusterer():
	...
	def get_cos_angle(self, doc1, doc2):
		return self.get_dot_prod(doc1, doc2) / (self.get_doc_length(doc1) * self.get_doc_length(doc2))


	def get_nearest_doc(self, i, doc):
		return sorted(self.doc_vecs.keys())[max(range(len(self.doc_vecs)), key=lambda k: self.cos_mat[i][k])]
	...
	def compute_cosine_matrix(self):
		for i, doci in enumerate(sorted(self.doc_vecs.keys())):
			for j, docj in enumerate(sorted(self.doc_vecs.keys())):
				if i == j:
					self.cos_mat[i][j] = -inf # Similar docs will have cos value as 1 and will be redundant in the calculation 
				else:
	...
	def fit(self, wordlist, docfile):
		...
		self.cos_mat = [[0 for i in range(len(self.doc_vecs))] for j in range(len(self.doc_vecs))]
