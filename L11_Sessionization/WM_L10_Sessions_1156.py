...
class Sessionizer():
	def __init__(self, h1, h2):
		self.h1 = h1
		self.h2 = h2
		self.session_table = pd.DataFrame(columns = ["session_no", 'session_ip', 'session_start_time', 'session_end_time'])
	...
	def parse(self, datecol=None, datetime_format=None):
		...
		self.data[datecol] = [datetime.datetime.strptime(dt, datetime_format) for dt in self.data[datecol]]
	...
	def sessionize(self):
		...
		for ip in data_ip.groups.keys():
			data_ip_time = data_ip.get_group(ip).groupby(['ip', pd.Grouper(key=self.datecol, freq= str(self.h1)+"Min")])
			for key in data_ip_time.groups.keys():
				try:
					...
					temp = {'session_no':sno, 'session_ip':key[0], "session_start_time":time_min, "session_end_time":time_max}
					self.session_table = self.session_table.append(pd.DataFrame(temp.copy(), index=[0]), ignore_index = True, sort=False)
					sno += 1
