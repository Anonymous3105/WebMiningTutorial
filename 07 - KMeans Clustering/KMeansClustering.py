...
class KMeans_Clusterer():
	...
	def fit(self, wordlist, docfile, input_type=None):
		...
		self.get_word_freq()
		self.freq_matrix = [
			[0 for i in range(self.k)] for i in range(len(self.doc_vecs.keys()))
		]
	...
	def get_doc_dist(self, doc1, doc2, metric="manhattan"):
		if metric == "manhattan":
			...
			return round(
				(sum((self.doc_vecs[doc1] - self.doc_vecs[doc2])**2))**(0.5), 4
			)
	...
	def update_cluster_centroids(self):
		for curr_clus in sorted(self.clusters.keys()):
			...
			# print(clus_doc_vecs)
			self.cluster_centroids[curr_clus] = np.around(
				np.mean(clus_doc_vecs, axis=0), 4
			)
	...
	def update_freq_matrix(self):
		for i, doci in enumerate(sorted(self.doc_vecs.keys())):
			for j, clusj in enumerate(sorted(self.clusters.keys())):
				self.freq_matrix[i][j] = self.get_doc_dist(
					self.doc_vecs[doci], self.cluster_centroids[clusj], metric="manhattan"
				)

	def cluster_init(self):
		self.clusters = {
			"Cluster" + str(i): [sorted(self.doclist.keys())[i]] for i in range(self.k)
		}
